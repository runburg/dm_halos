...
def integrated_j_factor(list, upper_bound, wave, plot=False):
    """Return the value of the integrated j-factor for wave annihilation to angle upper_bound in degrees."""
    temp_j = []
    # changeunits_to_gevcm = 37.96**2 * 10**(-18) * 1000 * 3.086 * 10**18
    ...
    for x in list:
        ...
        temp_j.append((x[0], 4*np.pi*r_s**3*rho_s**2/(d**2)*(4*np.pi * g_n * rho_s * r_s**2 / c**2)**(n/2.) * j_tot * changeunits_to_gevcm, d, r_s, rho_s))
    if len(list) == 1:
        return(temp_j)
    else:
        return weighted_median(temp_j, plot=plot)
    ...
    for ub in bounds:
        ...
        for dwarf in dwarflist:
            ...
            for wave in waves:
                ...
                #     plot = True
                l, a, u, d, r_s, rho_s = integrated_j_factor(data, ub, wave, plot=plot)
        ...
        outfile = "./j_factors/tot_j_factors/tot_j_fac_inclusive_" + angledict[ub]
