...
def dimensionless_integrated_j_factor():
    ...
    ub = np.logspace(-1, 1.5, num=10)

    for angle in ub:
        index = np.searchsorted(tildetheta, angle, side='left')
        limit = 1000
        jtot_s.append(integrate.quad(lambda y: y * jsfunc(y), 0, angle, points=tildetheta[:index], limit=limit, full_output=1)[0])
        jtot_p.append(integrate.quad(lambda y: y * jpfunc(y), 0, angle, points=tildetheta[:index], limit=limit, full_output=1)[0])
        jtot_d.append(integrate.quad(lambda y: y * jdfunc(y), 0, angle, points=tildetheta[:index], limit=limit, full_output=1)[0])
        jtot_som.append(integrate.quad(lambda y: y * jsomfunc(y), 0, angle, points=tildetheta[:index], limit=limit, full_output=1)[0])
