...
def hts(y):
    return y * mp.quad(lambda x: gtfunc(x)/(mp.sqrt(1-(y/x)**2)), y, rs[-1])[0]
...
#    return y * integrate.quad(interpolate.interp1d(rs, np.vectorize(lambda x: gfunc(x)/(mp.re(mp.sqrt(1.00000000001-(y/(x))**2)+0.00000000000000001)))(rs), kind = 'cubic'), y, rs[-1])[0]
    return y * mp.quad(lambda x: gfunc(x)/(mp.sqrt(1-(y/(x))**2)), y, rs[-1])[0]
...
#    return y * integrate.quad(hpint, y, rp[-1])[0]
    return y * mp.quad(lambda x: gpfunc(x)/(mp.sqrt(1-(y/(x))**2)), y, rp[-1])[0]
...
#    return y * integrate.quad(hint, y, rd[-1])[0]
    return y * mp.quad(lambda x: gdfunc(x)/(mp.sqrt(1-(y/(x))**2)), y, rd[-1])[0]
